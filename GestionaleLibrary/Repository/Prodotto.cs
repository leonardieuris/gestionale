...
namespace GestionaleLibrary.Repository
{
    public class Prodotti : IProdotti
    {
        ...
        public IEnumerable<Prodotto> GetAllProdotto()
        {
            ...
            {
                connection.Open();
                string sql = "select IdProdotto,NomeProdotto,Codice8,Categoria,Qta,Attivo from prodotto where attivo = 1";
                using (var command = new SqlCommand(sql, connection))
                {
        ...
        public Prodotto GetProdotto(int idProdotto)
        {
            ...
            {
                connection.Open();
                string sql = "select IdProdotto,NomeProdotto,Codice8,Categoria,Qta,Attivo from prodotto where idprodotto = @idprodotto";
                using (var command = new SqlCommand(sql, connection))
                {
                    command.Parameters.AddWithValue("@idprodotto", idProdotto);
                    ...
                    using (SqlDataReader dr = command.ExecuteReader())
                    {
                        ...
                        throw new ArgumentOutOfRangeException(nameof(idProdotto));
                    }
                }
            }
        }
        ...
        public IEnumerable<Prodotto> GetProdotto(string search)
        {
            ...
            {
                connection.Open();
                string sql = "select IdProdotto,NomeProdotto,Codice8,Categoria,Qta,Attivo from prodotto where nomeprodotto like @search or codice8 like @search ";
                using (var command = new SqlCommand(sql, connection))
                {
                    using (SqlDataReader dr = command.ExecuteReader())
                    {
